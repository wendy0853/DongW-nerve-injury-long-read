##RUN CODE: bash bsub_sqanti3_test.sh {SAMPLE_NAME} {DIR} {SRB}
...
SRB=$3
LSF_DOCKER_PRESERVE_ENVIRONMENT=false bsub  -G compute-jin810 -q general -n 16  -R 'rusage[mem=100GB]' -a 'docker(jinlab/sqanti3:vs1)' /bin/bash run_sqanti3.sh ${SAMPLE_NAME} ${DIR} ${SRB}
