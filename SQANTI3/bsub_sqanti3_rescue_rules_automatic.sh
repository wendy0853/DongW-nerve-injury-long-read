##RUN CODE: bash bsub_sqanti3_rescue_rules_automatic.sh {SAMPLE_NAME} {DIR}
...
#C7_Injury
DIR=$2
LSF_DOCKER_PRESERVE_ENVIRONMENT=false bsub  -G compute-jin810-t3 -q subscription -sla jin810_t3 -n 16  -R 'rusage[mem=100GB]' -a 'docker(jinlab/sqanti3:vs1)' /bin/bash run_sqanti3_rescue_rules_automatic.sh ${SAMPLE_NAME} ${DIR}
